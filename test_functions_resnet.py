...
def detection_test(model, vgg, test_dataloader, config):
    ...
    for data in test_dataloader:
        ...
        output_real = vgg(X)
        y_pred_1, y_pred_2, y_pred_3, y_pred_4 = output_pred[1], output_pred[2], output_pred[3], output_pred[4]
        y_1, y_2, y_3, y_4 = output_real[1], output_real[2], output_real[3], output_real[4]
        ...
        if direction_only:
            ...
            loss_4 = 1 - similarity_loss(y_pred_4.view(y_pred_4.shape[0], -1), y_4.view(y_4.shape[0], -1))
            ...
            abs_loss_4 = torch.mean((y_pred_4 - y_4) ** 2, dim=(1, 2, 3))
            loss_4 = 1 - similarity_loss(y_pred_4.view(y_pred_4.shape[0], -1), y_4.view(y_3.shape[0], -1))
            total_loss = loss_1 + loss_2 + loss_3 + loss_4 + lamda * (abs_loss_1 + abs_loss_2 + abs_loss_3 + abs_loss_4)
...
def localization_test(model, vgg, test_dataloader, ground_truth, config):
    ...
    class_name = config['normal_class']
    ...
    item,label = iter(test_dataloader).next()

    save_path = "./result/"+ class_name
    #print(test_dataloader[0])
    #print(len(grad))
    #print(len(ground_truth))
    #thresholds = [0.001 * i for i in range(1000)]
    #grad_t = 1.0 * (grad >= threshold)
    #grad_t = morphological_process(grad)
    
    print('len_grad',len(grad))
    for i in range(len(grad)-1):
        #print('i:',i)
        #ax = sns.heatmap(grad[i])
        fig_img, ax_img = plt.subplots(1, 3, figsize=(12, 4))
        fig_img.subplots_adjust(left=0, right=1, bottom=0, top=1)
        for ax_i in ax_img:
            ax_i.axes.xaxis.set_visible(False)
            ax_i.axes.yaxis.set_visible(False)
        #img=sns.heatmap(grad[i]).get_figure()
        ax_img[0].imshow(grad[i])
        ax_img[0].title.set_text('heatmap')
        ax_img[1].imshow(ground_truth[i])
        ax_img[1].title.set_text('ground_truth')
        ax_img[2].imshow(item[i].permute(1, 2, 0))
        #ax_img[2].imshow(np.transpose(items[i], (1, 2, 0)))
        ax_img[2].title.set_text('Image_origin')
        
        
        os.makedirs(os.path.join(save_path, 'images'), exist_ok=True)
        
        fig_img.savefig(os.path.join(save_path, 'images', f'localization_{class_name}_{i}.png'), dpi=100)
        #fig_img.savefig(f'svm_conf_2_{i}.png')
        fig_img.clf()
        #break
        # fig = img.get_figure()
        # fig.savefig(i)
...
def grad_calc(inputs, model, vgg, config):
    ...
    for i in range(inputs.shape[0]):
        ...
        abs_loss_4 = criterion(y_pred_4, y_4)
        loss_4 = torch.mean(1 - similarity_loss(y_pred_4.view(y_pred_4.shape[0], -1), y_4.view(y_4.shape[0], -1)))
        total_loss = loss_1 + loss_2 + loss_3 + loss_4 + lamda * (abs_loss_1 + abs_loss_2 + abs_loss_3 + abs_loss_4)
        model.zero_grad()
        total_loss.backward()
...
def gradients_localization(model, vgg, test_dataloader, config):
    ...
    for data in test_dataloader:
        ...
        temp = np.zeros((grad.shape[0], grad.shape[1], grad.shape[2], grad.shape[3]))
